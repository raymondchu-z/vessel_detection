...
hyp = {'giou': 3.54,  # giou loss gain
       ...
       'iou_t': 0.20,  # iou training threshold ??????????? Ground Truth ??? IoU<ignore_thresh ??????????????
...
def train(hyp):
    ...
    test_path = data_dict['valid'] # using test_path for valid set
    nc = 1 if opt.single_cls else int(data_dict['classes'])  # number of classes
    hyp['cls'] *= nc / 80  # update coco-tuned hyp['cls'] to current dataset ?cls loss gain???????
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-SeaShips.cfg', help='*.cfg path')
    parser.add_argument('--data', type=str, default='data/SeaShips.data', help='*.data path')
